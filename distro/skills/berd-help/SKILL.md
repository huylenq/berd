...
`cursor-agent`). `hermes-acp` is listed in Settings as unavailable until the
Goose pin on huylenq/goose registers that provider. A question about the app around the harness (where a
